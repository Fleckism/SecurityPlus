...
2.8 Summarize the basics of ( #A_D ) cryptographic #concepts
...
A message authentication code ([[MAC]]) provides an authentication and integrity mechanism by [[hash|hashing]] a combination of the message output and a shared secret key. The recipient can perform the same process using his or her copy of the secret key to verify the data. This type of authenticated encryption scheme is specified in a cipher suite as separate functions, such as "[[AES]] [[CBC]] with HMAC-SHA." Unfortunately, the implementation of this type of authenticated mode in AES CBC is vulnerable to a type of cryptographic attack called a padding oracle attack ([docs.microsoft.com/en-us/dotnet/standard/security/[[vulnerability|vulnerabilities]]-cbc-mode](https://docs.microsoft.com/en-us/dotnet/standard/security/[[vulnerability|vulnerabilities]]-cbc-mode)).
