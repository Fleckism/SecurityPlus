...
Routing is subject to numerous [[vulnerability |[[vulnerability|vulnerabilities]]]], including: #vulnerability 
...
-   Software exploits in the underlying operating system—Hardware routers (and switches) have an embedded operating system. For example, Cisco devices typically use the Internetwork Operating System (IOS). Something like IOS suffers from fewer exploitable [[vulnerability|vulnerabilities]] than full network operating systems. It has a reduced attack surface compared to a computer OS, such as Windows. 
