...
As well as analyzing adversary tools and behaviors, another source of threat intelligence is identifying [[vulnerability|vulnerabilities]] in OS, software application, and firmware code. Security researchers look for [[vulnerability|vulnerabilities]], often for the reward of bug bounties offered by the vendor. Lists of [[vulnerability|vulnerabilities]] are stored in databases such as Common [[vulnerability|vulnerabilities]] and Exposures (CVE), operated by Mitre ([cve.mitre.org](https://cve.mitre.org/)). Information about [[vulnerability|vulnerabilities]] is codified as signatures and scanning scripts that can be supplied as feeds to automated vulnerability scanning software.
